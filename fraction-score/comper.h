...
struct task_container
{
    ...
    {   
        ...
        bool exit = vtx_support[u] + pattern->get_cands()[u].size() - domain_finished[u] < grami.nsupport_;
...
public:
        ...
        else if(value == MATCH_FOUND)
        {
            for (ui j = 0; j < pattern->size(); ++j)
            {
                ...
                // for (auto j: auto_nodes)
                {
                    ...
                    if (it == tc->domain_matches[j].end())
                    {
                        tc->domain_matches[j].insert(matched_vertex);
                    
                        /**
                         * For fraction-score, if above insertion succeeds, 
                         * that means we find a new row instance which involves matched_vertex
                         */
                        double frac_agg = std::numeric_limits<double>::max();
                        ui degree = pattern->get_vertex_degree(j);
     ...
     */
    bool fraction_score_is_freq(Pattern *pattern)
    {
        double frac_min = std::numeric_limits<double>::max();
        for (ui i = 0; i < pattern->get_cands().size(); ++i)
        {
            double frac_sup = 0;
            for (VertexID v: pattern->get_cands()[i].candidate)
            {
                double frac_agg = std::numeric_limits<double>::max();

                ui degree = pattern->get_vertex_degree(i);
                for (ui k = 0; k < degree; ++k)
                {
